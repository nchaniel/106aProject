...
class LinearTrajectory(Trajectory):
    ...
    def target_pose(self, time):
        ...
        if t <= self.t_half:
            ...
            pos = self.start_position + 1/2 * self.acceleration * t**2 * self.unit_direction
        else:
            # TODO: Calculate the position of the end effector at time t,
            ...
            pos = self.start_position + (1/2 * self.acceleration * self.t_half**2 + self.v_max * (t - self.t_half) - 1/2 * self.acceleration * (t - self.t_half)**2) * self.unit_direction
...
class CircularTrajectory(Trajectory):
    ...
    def target_pose(self, time):
        ...
        if t <= self.t_half:
            ...
            theta = 1/2 * self.angular_acceleration * t**2
        else:
            # TODO: Calculate the ANGLE of the end effector at time t,
            # For the second half of the trajectory, maintain a constant acceleration
            # Hint: Calculate the remaining angle to the goal position.

            theta = 1/2 * self.angular_acceleration * self.t_half**2 + self.angular_v_max * (t - self.t_half) - 1/2 * self.angular_acceleration * (t - self.t_half)**2
        '''
        OFFSET = np.pi/4
        theta = (t/self.total_time)*2*np.pi - OFFSET
        ...
        tilt_angle = np.arctan2(self.radius, HEIGHT)
