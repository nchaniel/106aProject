...
def triangulate_dlt(
    ...
    """
    Weighted DLT triangulation from ≥2 calibrated views.

    centroids_uv: [(u, v), ...] pixel centroid of the object per view
    poses_w2c:    [4×4, ...] world-to-camera transforms (same world frame)
    weights:      per-view reliability weights (e.g. mask pixel count). Uniform if None.
    Returns:      (3,) point in that world frame, in the same units as the
                  translations in poses_w2c
    """
    if weights is None:
        weights = [1.0] * len(centroids_uv)
    A = []
    for (u, v), T, w in zip(centroids_uv, poses_w2c, weights):
        P = np.array([[fx, 0, cx, 0],
                      [0, fy, cy, 0],
                      [0,  0,  1, 0]], dtype=np.float64) @ T.astype(np.float64)
        s = np.sqrt(max(w, 1e-8))
        A.append((u * P[2] - P[0]) * s)
        A.append((v * P[2] - P[1]) * s)
...
def triangulate_ransac(
    ...
    """
    RANSAC over views: rejects occluded/noisy detections via reprojection error,
    then re-fits with weighted DLT on the inlier set (weighted by mask area).
    Returns (X_world, inlier_indices). Falls back to all views if fewer than 2
    inliers are found.
    """
    n = len(centroids_uv)
    if n < 2:
        return None, []
    if n == 2:
        X = triangulate_dlt(centroids_uv, poses_w2c, fx, fy, cx, cy, weights=mask_areas)
        return X, [0, 1]
    ...
    for _ in range(n_iter):
        ...
        try:
            X = triangulate_dlt(
                [centroids_uv[i], centroids_uv[j]],
                [poses_w2c[i],    poses_w2c[j]],
                fx, fy, cx, cy,
            )
        except Exception:
            continue
        ...
        inliers = []
        for k, (uv, T) in enumerate(zip(centroids_uv, poses_w2c)):
            p = T @ np.append(X, 1.0)
            if p[2] <= 0:
                continue
            u_p = fx * p[0] / p[2] + cx
            v_p = fy * p[1] / p[2] + cy
            if np.hypot(u_p - uv[0], v_p - uv[1]) < inlier_thresh_px:
                inliers.append(k)
    ...
    if len(best_inliers) < 2:
        best_inliers = list(range(n))  # fallback: use all views

    inlier_uvs   = [centroids_uv[k] for k in best_inliers]
    inlier_poses = [poses_w2c[k]    for k in best_inliers]
    inlier_areas = [mask_areas[k]   for k in best_inliers]
    X = triangulate_dlt(inlier_uvs, inlier_poses, fx, fy, cx, cy, weights=inlier_areas)
...
class PoseEstimator:
    ...
    def build_databases(self, force_rebuild: bool = False):
        if self.position_only:
            print("Position-only mode — skipping reference database build.")
            return
        print("\n══════════════════════════════════════════")
    ...
    def process_scene(
        ...
        """
        Full pipeline: load masks → estimate pose for every detected object.

        mask_dir:     path to a folder of pre-computed masks from YOLO.
        ...
        if self.position_only:
            return self._scene_centroids_only(image, all_masks, fx, fy)
    ...
    def process_multi_view(
        ...
        mask_dirs: list of pre-computed mask directories (one per view).
        ...
        if self.position_only:
            return self._multi_view_position_only(
                views, mask_dirs, seg_name_map, fx, fy, cx, cy)
    ...
    def _multi_view_position_only(
        self, views, mask_dirs, seg_name_map, fx, fy, cx, cy,
        ransac_thresh_px: float = 8.0,
    ) -> list[dict]:
        """
        RANSAC + weighted DLT triangulation over all views (position-only mode).
        Views are weighted by mask area; occluded views are rejected by RANSAC.
        """
        # Collect centroid observations keyed by (object_name, instance_idx)
        by_key: dict[tuple, list[dict]] = {}
        for i, (image, cam_T_world) in enumerate(views):
            print(f"\n── View {i + 1}/{len(views)} ──")
            all_masks = load_masks_from_dir(mask_dirs[i], seg_name_map)
            for r in self._scene_centroids_only(image, all_masks, fx, fy):
                key = (r["object_name"], r["instance_idx"])
                by_key.setdefault(key, []).append({**r, "_cam_T_world": cam_T_world})

        final: list[dict] = []
        view0_cam = views[0][1]
        for (name, _), detections in by_key.items():
            uvs   = [d["centroid_uv"]   for d in detections]
            poses = [d["_cam_T_world"]  for d in detections]
            areas = [d["mask_area"]     for d in detections]

            # Use the view-0 detection as the base (mask, position_m, etc.)
            base = next(
                (d for d in detections if np.array_equal(d["_cam_T_world"], view0_cam)),
                max(detections, key=lambda d: d["mask_area"]),
            )
            result = {k: v for k, v in base.items() if k != "_cam_T_world"}

            if len(uvs) >= 2:
                X, inliers = triangulate_ransac(
                    uvs, poses, areas, fx, fy, cx, cy,
                    inlier_thresh_px=ransac_thresh_px,
                )
                result["triangulated_position_m"] = X
                result["n_views_triangulated"]    = len(inliers)
                result["n_views_detected"]        = len(uvs)
                print(f"  [{name}] triangulated from {len(inliers)}/{len(uvs)} views: "
                      f"{X.round(4)} m")
            else:
                print(f"  [{name}] seen in only 1 view — using apparent-size estimate")

            result["confidence"] = min(1.0, len(uvs) / max(len(views), 1))
            final.append(result)

        return final

    def _scene_centroids_only(self, image, all_masks, fx, fy) -> list[dict]:
        """Position-only scene processing: median centroid + apparent-size depth per mask."""
        H, W = image.shape[:2]
        cx, cy = W / 2.0, H / 2.0
        results = []
        for name, masks in all_masks.items():
            ...
            for inst_idx, mask in enumerate(masks):
                ...
                if not len(ys):
                    continue
                # Median centroid: robust to partial occlusion clipping mask edges
                u_c  = float(np.median(xs))
                v_c  = float(np.median(ys))
                area = float(len(xs))
                result: dict = {
                    "object_name":  name,
                    "instance_idx": inst_idx,
                    ...
                    "centroid_uv":  (u_c, v_c),
                    "mask_area":    area,
                    "diameter_m":   cfg.diameter_m if cfg else None,
                }
                if cfg is not None and cfg.diameter_m is not None:
                    r_px = max(1.0, float(np.sqrt(area / np.pi)))
                    z_m  = fx * (cfg.diameter_m / 2.0) / r_px
                    result["position_m"] = np.array([
                        (u_c - cx) / fx * z_m,
                        (v_c - cy) / fy * z_m,
                        z_m,
