...
class RealSensePCSubscriber(Node):
    def __init__(self):
        ...
        self.max_y = float(self.declare_parameter('max_y', 0.7).value) # higher = towards computer
        #self.max_y = float(self.declare_parameter('max_y', 0.5).value)
        self.min_y = float(self.declare_parameter('min_y', 0.45).value) #



        self.min_z = float(self.declare_parameter('min_z', -0.17).value)# more neg = closer to table
    ...
    def pointcloud_callback(self, msg: PointCloud2):
        ...
        filtered_points = points_base[(points_base[:,1] >= self.min_y) & (points_base[:,1] <= self.max_y) & (points_base[:,2] >= self.min_z) & (points_base[:,2] <= self.max_z)]
